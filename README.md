# windows-settings

## **Crtl + Space**で入力言語を切り替えられるようにする

1. タスクバーのIMEボタン(「あ」もしくは「A」という表記)を右クリックし設定を選択
2. **キーとタッチとのカスタマイズ > キーの割り当て** をオンにして、Ctrl + Space に **IME-オン/オフ** を割り当てる

## caps lockにctrlを割り当てる

> [!NOTE]
> PowerToys の Keyboard Manager で実施するのが理想だが、2026/1/27現在、caps lock キーなどのトグルキーを変更するとバグってしまうため不採用。
> [!WARNING]
> Ctrl2Cap は Microsoft のツールであるため信頼できるが、管理者権限でのインストールが必要

1. [Ctrl2Cap](https://learn.microsoft.com/ja-jp/sysinternals/downloads/ctrl2cap)をダウンロードし解凍
2. 管理者権限で開いたPower Shellで以下のコマンドを実行。

```shell
cd {{解凍したフォルダ}}
.\ctrl2cap /install
```

## カーソルの色を反転して視認性向上

1. 設定アプリを起動
2. **アクセシビリティ > マウス ポインターとタッチ > マウス ポインターのスタイル** に反転色を割り当てる

## Power Toysで便利機能を設定する

1. PowerToysアプリを起動
2. 以下の機能をオンにする

### コマンド パレット

コマンド、アプリ、開発ツールに 1 つの高速ソリューションからアクセスするために使用できるクイックランチャー ユーティリティ

### CursorWarp

マウスポインターが画面の端に到達した際、反対側の端からワープして出現させることができる

### マウスの検索

**アクティブ化の方法** に **マウスをシェイクする** を設定すると、マウスを素早く動かしたときにポインターがハイライトされるので、ポインターを見失った時に見つけやすい。

### Image Resizer

1 つ以上のイメージ ファイルを右クリックし、**ImageResizer でサイズ変更** で 一括サイズ変更可能

### PowerRename

一度に複数のファイルの名前をすばやく変更できる一括名前変更ツール

## 本体JISキーボード と 外付けUSキーボード を併用できるようにする

> [!CAUTION]
> 個人が作成したツールであるため利用には確認が必要となる。

1. [USkey2JP](https://ms-soft.jp/tools/uskey2jp/)からzipファイルをダウンロードし、解凍
2. USkey2JP.exeを実行

## IMEにユーザー辞書を登録する

> [!NOTE]
> IMEユーザー辞書ファイルは、**タブ区切りのテキストファイル**かつ**Shift JIS**である必要がある。設定方法はネットを参照すること。

1. `./IMEユーザー辞書.txt`に、個人名、会社名、部署名など不足している単語を追記する
2. タスクバーのIMEボタン(「あ」もしくは「A」という表記)を右クリックし**単語の追加**を選択
3. 左下の**ユーザー辞書ツール**を選択
4. **ツール**タブの**テキストファイルからの登録**を選択
5. `./IMEユーザー辞書.txt`をアップロード
